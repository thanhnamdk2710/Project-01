$_DOMAIN = 'http://localhost:7000/Project/Project-01/admin/';
...
/*================================================
=== Xóa nhìu chuyên mục cùng lúc
================================================*/
$('#del_cate_list').on('click', function() {
    $confirm = confirm('Bạn có chắc chắn muốn xóa các chuyên mục đã chọn!');
    if ($confirm == true) {
        $id_cate = [];
        ...
        if ($id_cate.length === 0) {
            alert('Vui lòng chọn ít nhất 1 chuyên mục!');
        } else {
            $.ajax({
                url: $_DOMAIN + 'categories.php',
                type: 'POST',
...
/*================================================
=== Xóa chuyên mục chỉ định trong bảng danh sách
================================================*/
...
/*================================================
=== Xóa chuyên mục chỉ định trong trang chỉnh sửa
================================================*/
...
function preUpImg () {
    ...
    if (img_up != '') {
        ...
        for (var i = 0; i <= count_img_up - 1; i++) {
            $('#formUpImg .box-pre-img').append('<img src="'+ URL.createObjectURL(event.target.files[i]) +'" style="border: 1px solid #cacaca; width: 50px; height: 50px; margin-bottom: 5px;" />');
        }
    }
...
/*================================================
=== Xóa nhiều hình ảnh cùng 1 lúc
================================================*/
$('#del_img_list').on('click', function() {
    $confirm = confirm('Bạn có chắc muốn xóa các hình ảnh đã chọn không');
    if ($confirm == true) {
        $id_img = [];
...
/*================================================
=== Xóa hình ảnh chỉ định
================================================*/
$('.del-img').on('click', function() {
    $confirm = confirm('Bạn có chắc muốn xóa ảnh này?');
    if ($confirm == true) {
...
//     $body_add_post = CKEDITOR.instances['body_edit_post'].getData();
...
/*================================================
=== Xóa nhiều bài viết cùng 1 lúc
================================================*/
$('#del_post_list').on('click', function() {
    $confirm = confirm('Bạn có chắc muốn xóa các vài viết đã chọn không?');
    if ($confirm == true) {
        $id_post = [];
...
/*================================================
=== Xóa bài viết chỉ định trong bảng chỉnh sửa
================================================*/
...
$('#unlock_acc_list').on('click', function() {
    $confirm = confirm('Bạn có chắc muốn mở khóa các tài khoản đã chọn không?');
    if ($confirm == true) {
        $id_acc = [];
...
$('#del_acc_list').on('click', function() {
    $confirm = confirm('Bạn có chắc muốn xóa các tài khoản đã chọn không?');
    if ($confirm == true) {
        $id_acc = [];
...
function preUpAvt () {
    ...
    if (img_avt != '') {
        $('#formUpAvt .box-pre-img').html('<p><strong>Ảnh xem trước</strong></p>');
        $('#formUpAvt .box-pre-img').removeClass('hidden');
        $('#formUpAvt .box-pre-img').append('<img src="'+ URL.createObjectURL(event.target.files[0]) +'" style="border:1px solid #ddd; width:50px; height:80px; margin-right:5px; margin-bottom: 5px;" />');
...
$('#formUpAvt').submit(function (e) {
    ...
    if (img_avt) {
        ...
        // Nếu lỗi về định dạng ảnh
            ...
            $('#formUpAvt button[type=submit]').html('Upload');
            $('#formUpAvt .alert-danger').removeClass('hidden');
            $('#formUpAvt .alert-danger').html('Tệp đã chọn không đúng định dạng cho phép.');
        } else {
            $(this).ajaxSubmit({
                beforeSubmit: function () {
                    ...
                    $('#formUpAvt button[type=submit]').html('Upload');
                    $('#formUpAvt .alert-danger').removeClass('hidden');
                    $('#formUpAvt .alert-danger').html('Không thể Upload hình ảnh vào lúc này, hãy thử lại.');
                },
                resetForm: true
            });
            return false;
...
$('#del_avt').on('click', function() {
    $confirm = confirm('Bạn có chắc muốn xóa ảnh đại diện này không?');
    if ($confirm == true) {
        $.ajax({
    ...
    if ($dn_update && $email_update) {
        $.ajax({
            ...
            data: {
                ...
                $('#formUpdateInfo .alert').removeClass('hidden');
                $('#formUpdateInfo .alert').html('Đã có lỗi xãry ra, vui lòng thử lại.');
            }
        });
    ...
    if ($old_pw_change && $new_pw_change && $re_new_pw_change) {
        $.ajax({
            ...
            data: {
                ...
                $('#formChangePw .alert').removeClass('hidden');
                $('#formChangePw .alert').html('Đã xảy ra lỗi, xin thử lại sau.');
            }
        });
